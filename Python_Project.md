...
- **Что хранится**
  - `faculties` — справочник факультетов. В коде есть сущность `FacultyModel`, у которой только идентификатор и название, а связь один-ко-многим с пользователями нужна, чтобы быстро вытянуть название факультета при показе анкеты.
  ...
  - `likes` — реакции на анкеты. У записи есть отправитель, получатель, флаг лайка или скипа и время. Уникальное ограничение на пару айдишников не дает дважды сохранить одну и ту же реакцию в разных обработчиках.
...
- **Как создается и заполняется база**
  `init_db` в `domain/db.py` поднимает схему через `Base.metadata.create_all`, а потом проверяет, есть ли дефолтные факультеты. Если их нет, добавляет стартовый набор (`ФКН`, `ФЭН`, `ВШБ`, `ФГН`) и коммитит изменения. Так база готова к работе сразу после первого запуска контейнера.

- **Как идет работа с данными**
  - Репозитории закрывают детали SQLAlchemy за простыми методами. `FacultyRepository` отдает список факультетов или конкретный объект, чтобы клавиатура регистрации всегда брала актуальные значения.
  - `UserRepository` отвечает за поиск пользователя по Telegram ID или по внутреннему айди и за `upsert_user`. Если запись не найдена, создается новая, иначе обновляются поля анкеты и фото, после чего сессия коммитится и модель обновляется в памяти. Так регистрация и редактирование используют один и тот же путь.
  - `MatchRepository` занимается всем, что связано с поиском и мэтчами. Сначала из лайков собирается список уже оцененных анкет, чтобы не показывать их снова. Затем выполняется приоритетный поиск: через соединение таблицы `users` с `likes` вытаскиваются те, кто уже поставил лайк текущему пользователю и совпадает по полу и предпочтениям. Это соединение нужно, чтобы в одном запросе увидеть и анкету, и факт лайка на нее, и сразу вернуть кандидата с повышенным приоритетом. Если таких нет, берется случайная анкета с подходящими параметрами. При выставлении реакции проверяется, была ли встречная симпатия: если оба поставили лайк, создается запись в `matches` (с сортировкой айди для защиты от дублей).
  - При получении списка мэтчей сначала считается общее количество, а затем вытягивается нужная страница. Для каждой записи подтягивается анкета второй стороны, чтобы сразу показать возраст, пол, факультет и описание без дополнительных запросов.

- **Зачем такой порядок кандидатов и мэтчей**
  Сначала показываются те, кто уже проявил интерес к текущему пользователю, чтобы диалог не зависал в очереди и быстрее давал взаимные мэтчи. Хранение пар с отсортированными идентификаторами упрощает проверку уникальности: лайк в любую сторону приводит к одной и той же записи, значит статистика и выдача не дублируются.
